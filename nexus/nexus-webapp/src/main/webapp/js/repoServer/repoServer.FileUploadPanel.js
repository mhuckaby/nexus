...
Sonatype.repoServer.ArtifactUploadPanel = function(config) {
  ...
  this.uploadModeStore = this.uploadModeStore ? this.uploadModeStore : new Ext.data.SimpleStore({
        ...
        data : [['pom', 'From POM'], ['gav', 'GAV Parameters']]
      });
...
Ext.extend(Sonatype.repoServer.ArtifactUploadPanel, Ext.FormPanel, {
      ...
      doUpload : function() {
        ...
        if (hasArtifacts)
        {
          ...
          this.createUploadForm(treePanel, treePanel.root.childNodes[this.currentChildNode].attributes.payload.fileInput, treePanel.root.childNodes[this.currentChildNode].attributes.payload.classifier,
              treePanel.root.childNodes[this.currentChildNode].attributes.payload.extension, this.currentChildNode == (treePanel.root.childNodes.length - 1));
        }
                  ...
                  if (statusResp.data)
                  {
                    ...
                    else
                    {
                      cardPanel.add({
                            ...
                            items : [{
                                  border : false,
                                  html : '<div class="little-padding">' + 'Artifact deployment is disabled for ' + rec.data.name + '.<br /><br />' + 'You can enable it in the "Access Settings" section of the ' + 'repository configuration.</div>'
                                }]
                          });
                    }
